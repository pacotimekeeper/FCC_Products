...
@app begin
    ...
    @in manstatuses = unique(gdf.手動狀態) |> sort
    @in selectedManStatuses = String[]
    
    @in notastatuses = unique(gdf.NOTA_狀態) |> sort
    @in selectedNotaStatuses = String[]
    ...
    for (k, v) in [ (suppliers, selectedsuppliers),
                    ...
                    (notaManStatuses, selectedNotaManStatuses)]
        if isempty(v)
            @in v = ["Select"]
        else
            @in v = [first(v)]
    ...
    # @onbutton searchtext, selectedsuppliers, selectedcustomers, selectedstatuses, selectedManStatuses, selectedNotaStatuses, selectedNotaManStatuses begin
        ...
        mstatusmask = map(x -> isempty(selectedManStatuses) || x in selectedManStatuses, gdf.手動狀態)

        notaStatusMask = map(x -> isempty(selectedNotaStatuses) || x in selectedNotaStatuses, gdf.NOTA_狀態)
        notaManStatusMask = map(x -> isempty(selectedNotaManStatuses) || x in selectedNotaManStatuses, gdf.NOTA_手動狀態)
